...
class PumpFunTokenFetcher {
  ...
  async getTokenMetadata(tokenAddress: string): Promise<TokenMetadata | null> {
    ...
    try {
      ...
      console.log("ERROR FROM getTokenMetadata call\n", error);
   ...
   * @param {string} bondingCurveAddress - target bonding curve
   */
  async getDataWithBondingCurveAddress(bondingCurveAddress: string) {
    ...
    if (!tokenAddress) {
      throw new Error(
        `There was a problem getting the token address for ${bondingCurveAddress}`
      );
    }
    ...
    if (!tokenData) {
      throw new Error(
        `There was a problem getting the token data for ${tokenAddress}`
      );
    }
  ...
  async collectBondingCurves() {
    ...
    allBondingAddress.forEach((bonding) => {
      bondingAddressList.push(bonding.pubkey);
    });

    fs.writeFile(
      "bonding.json",
      JSON.stringify(bondingAddressList, null, 2),
      "utf-8"
    );
  ...
  async updateTokenList() {
    ...
    try {
      ...
      console.log(
        `📋 Found ${existingAddresses.length} previously processed addresses`
      );
    } catch (error) {
      console.log(
        "📋 No existing bonding_addresses.json file found, processing all addresses"
      );
    }
    ...
    for (let i = 0; i < newAddresses.length; i++) {
      try {
        ...
        if (
          tokenBatch.length >= CACHE_INTERVAL ||
    ...
    for (let i = 0; i < data.length; i++) {
      try {
        ...
        if (tokenBatch.length >= CACHE_INTERVAL || i === data.length - 1) {
          ...
          console.log(
            `📊 Progress: ${processedCount}/${data.length} tokens processed`
          );
        }
      } catch (error) {
  ...
  async appendToCache(tokens: any[], CACHE_FILE: string) {
    try {
      ...
      console.log(
        `✅ Cached ${tokens.length} tokens. Total cached: ${updatedData.length}`
      );
    } catch (error) {
